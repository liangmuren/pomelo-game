...
var config = pomelo.app.get('tencentSMS');
...
function singleSmsSend(msgType, nationCode, phoneNumber, msg, extend, ext, cb) {
    ...
    function callback(error, response, body) {
        if (!error && response.statusCode === statusCode.MESSAGE.OK) {
            cb(body);
        } else if (!error && response.statusCode !== statusCode.MESSAGE.OK) {
            retObj = {
                result: -1,
    ...
    request({
        url: singleSmsUrl + '?sdkappid=' + config.AppId + '&random=' + rand,
        method: 'POST',
        json: false,
        headers: {
...
function multiSmsSend(msgType, nationCode, phoneNumbers, msg, extend, ext, cb) {
    ...
    request({
        url: multiSmsUrl + '?sdkappid=' + config.AppId + '&random=' + rand,
        method: 'POST',
        json: false,
        headers: {
            'Content-Type': 'application/json'
